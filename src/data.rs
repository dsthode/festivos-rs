...
impl Data {
    pub fn new() -> Result<Data, DataError> {
        ...
        let json_data: Vec<HashMap<String, Vec<String>>> =
            serde_json::from_str(&file_data).expect("failed to decode json");
            ...
            for (key, value) in item {
                if rx.is_match(&key) {
                    ...
                    let month = caps
                        ...
                        .as_str()
                        .parse::<u32>()
                        .expect("failed parsing month");
                    let day = caps
                        .get(3)
                        .unwrap()
                        .as_str()
                        .parse::<u32>()
                        .expect("failed parsing day");
    ...
    fn add_date(&mut self, year: u32, month: u32, day: u32, locations: Vec<String>) {
        ...
        self.data
            ...
            .and_modify(|items| {
                locations
                    .iter()
                    .for_each(|location| items.push(location.to_string()));
            })
            .or_insert(locations);
    }
