...
impl <'server>Server<'server> where 'server: 'static {
  ...
  fn router(&self) -> Router {
    let server_data = ServerData{ data: self.data.clone() };
  ...
  fn festivo_handler(state: State) -> (State, Response<Body>) {
    ...
    let locs = server_data.data.get_date(path_params.year, path_params.month, path_params.day);
    let mut code;
    ...
    (state, Response::builder().status(code).body(Body::empty()).unwrap())
  }
}
